...
async def create_user(user: UserRegistrationRequestSchema, db: AsyncSession = Depends(get_db)):
    try:
        ...
        return HTTPException(status_code=500, detail="An error occurred during user creation.")
...
async def login_request(
    ...
    try:
        ...
        if not user.is_active:
            return HTTPException(status_code=403, detail="User account is not activated.")
        data = {
            "sub": user.email,
            "user_id": user.id
        }
...
async def refresh_access_token(
    ...
    try:
        ...
        return HTTPException(status_code=400, detail="Token has expired.")
